...
def margin(x,y,th):
    norm_theta = np.sqrt(np.sum(np.square(th.T), axis=1)) # this is the norm of theta
    theta_tr_x = th.T @ x # so the x is the one that changes for every value while the theta remains the same.
    return theta_tr_x/norm_theta
def length_many(col_v):
    return np.sqrt(np.sum(np.square(col_v), axis=1))
